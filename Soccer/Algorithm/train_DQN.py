...
# 유니티 환경 경로 
game = "Soccer"
env_name = "../Build/Soccer"

# 모델 저장 및 불러오기 경로
save_path = "../saved_models/" + game + "/" + date_time + "_DQN"
load_path = "../saved_models/" + game + "//model/model"
...
class Model_s():
    def __init__(self, model_name):
        ...
        with tf.variable_scope(name_or_scope=model_name):
            ...
            self.Q_Out = tf.layers.dense(self.fc3,action_size_s,activation=None)
        self.predict = tf.argmax(self.Q_Out, 1)

        self.target_Q = tf.placeholder(shape=[None, action_size_s], dtype=tf.float32)
...
class Model_g():
    def __init__(self, model_name):
        ...
        with tf.variable_scope(name_or_scope=model_name):
            ...
            self.fc2 = tf.layers.dense(self.fc1,512,activation=tf.nn.relu)
            self.fc3 = tf.layers.dense(self.fc2,512,activation=tf.nn.relu)
            self.Q_Out = tf.layers.dense(self.fc3,action_size_g,activation=None)
        self.predict = tf.argmax(self.Q_Out, 1)

        self.target_Q = tf.placeholder(shape=[None, action_size_g], dtype=tf.float32)
...
class DQNAgent():
    def __init__(self):
        self.model1 = Model_s("Q1")
        self.target_model1 = Model_s("target1")
        self.model2 = Model_g("Q2")
        self.target_model2 = Model_g("target2")
    ...
    def get_action(self, state1, state2):
        if self.epsilon > np.random.rand():
            # 랜덤하게 액션 결정
            random_action1 = np.random.randint(0, action_size_s)
            random_action2 = np.random.randint(0, action_size_g)
...
if __name__ == '__main__':
    ...
    for episode in range(run_episode + test_episode):
        ...
        while not done:
            ...
            env_info = env.step(vector_action = {brain_name1: action1s, brain_name2: action2s})
